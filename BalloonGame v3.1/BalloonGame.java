...
public class BalloonGame {
    ...
    private ArrayList <Integer> xCordList = new ArrayList<Integer>(); // List of possible x cords of the balloons
    private ArrayList <Integer> yCordList = new ArrayList<Integer>(); // List of possible y cords of the balloons
    // (initially empty)
    ...
    public void restartGame(){
        ...
        balloons.clear(); 
        this.score = 0; 
        for (int i = 50; i <= 550; i = i + 20){ // fills the array of x and y coordinates 
            xCordList.add(i);
            yCordList.add(i); 
        }
        
        for (int i = 0; i < num_balloons; i++){
            int xIndex = (int)(Math.random() * (xCordList.size()-1)); // selects an x cord at random from the arraylist
            setX = xCordList.get(xIndex); 
            xCordList.remove(xIndex); // removes that x cord from the array list so that there are no overlaps
            
            int yIndex = (int)(Math.random() * (yCordList.size()-1)); // selects a y cord at random from the arraylist
            setY = yCordList.get(yIndex); 
            yCordList.remove(yIndex); // removes that y cord from the array list so that there are no overlaps
            
            balloons.add(new Balloon(setX,setY)); // creates a new balloon
        }
        for (Balloon b: balloons){ // draws balloons
            b.draw();
        }
    }
    ...
    public void  doMouse(String action, double x, double y){
        ...
        if (action.equals("released")){
            ...
            for (Balloon b: balloons){
                ...
                for (Balloon b2: balloons){ //checks if every balloon is touching another balloon
                    ...
                    if (b != b2 && touching == true){
                        if (b.explode == true){ // if it is a black balloon it is an 'exploding' balloon 
                            for(Balloon b3: balloons){
                                double xCord = b3.centerX ; 
                                double yCord = b3.centerY; 
                                double dist = b.distance(xCord, yCord);
                                if (dist < b.radius * 7){ // if the other balloons are close enough they will be popped by the exploding balloon
                                    b3.burst(); 
                                    b3.draw(); 
                                }
                            }
            ...
            score = area; 
            UI.println(playing); 
            if (playing == false){ // if all of the balloons have been popped
                lockScore(); 
